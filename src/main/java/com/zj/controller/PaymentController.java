...
import com.zj.enums.PayMethod;
import com.zj.enums.PaymentStatus;
import com.zj.pojo.Orders;
import com.zj.pojo.bo.MerchantOrdersBO;
import com.zj.pojo.vo.PaymentInfoVO;
import com.zj.resource.AliPayResource;
import com.zj.resource.WXPayResource;
import com.zj.service.PaymentOrderService;
import com.zj.utils.CurrencyUtils;
import com.zj.utils.ZjJSONResult;
import com.zj.utils.RedisOperator;
import com.zj.wx.entity.PreOrderResult;
import com.zj.wx.service.WxOrderService;
...
public class PaymentController {
	...
	public ZjJSONResult createMerchantOrder(@RequestBody MerchantOrdersBO merchantOrdersBO, HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		if (StringUtils.isBlank(merchantOrderId)) {
			return ZjJSONResult.errorMsg("参数[orderId]不能为空");
		}
		if (StringUtils.isBlank(merchantUserId)) {
			return ZjJSONResult.errorMsg("参数[userId]不能为空");
		}
		if (amount == null || amount < 1) {
			return ZjJSONResult.errorMsg("参数[realPayAmount]不能为空并且不能小于1");
		}
		if (payMethod == null) {
			return ZjJSONResult.errorMsg("参数[payMethod]不能为空并且不能小于1");
		}
		if (payMethod != PayMethod.WEIXIN.type && payMethod != PayMethod.ALIPAY.type) {
			return ZjJSONResult.errorMsg("参数[payMethod]目前只支持微信支付或支付宝支付");
		}
		if (StringUtils.isBlank(returnUrl)) {
			return ZjJSONResult.errorMsg("参数[returnUrl]不能为空");
		}
		...
		try {
			...
			e.printStackTrace();
			ZjJSONResult.errorException(e.getMessage());
		}

		if (isSuccess) {
			return ZjJSONResult.ok("商户订单创建成功！");
		} else {
			return ZjJSONResult.errorMsg("商户订单创建失败，请重试...");
		}
	}
	...
	public ZjJSONResult getPaymentCenterOrderInfo(String merchantOrderId, String merchantUserId) {

		if (StringUtils.isBlank(merchantOrderId) || StringUtils.isBlank(merchantUserId)) {
			return ZjJSONResult.errorMsg("查询参数不能为空！");
		}

		Orders orderInfo = paymentOrderService.queryOrderInfo(merchantUserId, merchantOrderId);

		return ZjJSONResult.ok(orderInfo);
	}
...
//	@GetMapping(value="/getWXPayQRCode")
	...
	public ZjJSONResult getWXPayQRCode(String merchantOrderId, String merchantUserId) throws Exception{
...
//				String total_fee = "1";	// 测试用 1分钱
			...
			return ZjJSONResult.ok(paymentInfoVO);
		} else {
			return ZjJSONResult.errorMsg("该订单不存在，或已经支付");
		}
	}
	...
	public ZjJSONResult goAlipay(String merchantOrderId, String merchantUserId) throws Exception{
...
//    	String total_amount = "0.01";	// 测试用 1分钱
		...
		return ZjJSONResult.ok(alipayForm);
	}
